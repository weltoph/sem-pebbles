...
\begin{frame}
  ...
  \alt<+>{
    \begin{theorem}[Reingold]
      There is an algorithm $A$ with $\mathcal{O}(n)$ space requirement producing
      a universal exploration sequence for every regular graph with $n$ vertices.
    \end{theorem}
  }{
    \begin{theorem}
  ...
  \begin{columns}
    ...
    \end{itemize}
    \column{0.5\textwidth}
    \uncover<4->{\resizebox{\textwidth}{!}{\input{tikz/regsim}}}
  \end{columns}
\end{frame}
