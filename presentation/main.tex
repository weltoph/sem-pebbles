        ...
        \uncover<2-2>{
          \begin{itemize}
            \item Construct $\mathcal{M}'$ which simulates $\mathcal{M}$ for
              $3n\cdot c$ steps where $c$ is the number of configurations of
              $M$
...
\begin{frame}
  ...
  \begin{itemize}
    \item Simulation of $M_{\text{Walk}}$ which explores at least $2^{m_{1}}$
      distinct vertices \uncover<2->{\checkmark}
    \item Identify \emph{distinct} vertices of the walk \uncover<3->{\checkmark}
    \item Read from and write to simulated tape \uncover<4->{\checkmark}
    \item Preserve tape content through steps of simulated agent \uncover<5->{\checkmark}
  \end{itemize}
  \uncover<6->{
    \begin{theorem}
      There is a constant $c\in\mathcal{N}$, such that for any Graph $G$
